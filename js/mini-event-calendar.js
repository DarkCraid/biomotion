(function( $ ) {
    ...
    $.fn.miniEventCalendar = $.fn.MEC = function(options) {
        ...
        event_title.text("");
		events_link.text("VER TODOS LOS REGISTROS");
		...
		let checkData = data => {
			return new Promise((resolve,reject) => {
				if(data)
					...
					reject('Sin pacientes en esta fecha');
		    });
		}

		mini_cal.on("click, focusin", ".a-date", function(){
		    if(!$(this).hasClass('blurred')){
		    	checkData($(this).data('event'))
		    		.then(data => {
		    			showChart(data);
		    		})
		    		.catch(err => {
		...
		function showEvent(event){
			if(event && event !== null && event !== undefined){
				...
				events_link.text("VER REGISTRO");
				events_link.attr("href", '#chart');
			}else{
				event_title.text("");
				events_link.text("VER TODOS LOS REGISTROS");
				events_link.attr("href", settings.calendar_link);
